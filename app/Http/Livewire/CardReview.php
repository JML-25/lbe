...
class CardReview extends Component
{
    ...
    // Filtres de sélection des fiches
    public string $filterReference = '';
    public string $filterPeriodFrom = '';
    public string $filterPeriodTo = '';
    public string $randomCount = '';   // nombre de fiches aléatoires (string pour le binding)

    // État de la révision
    public bool $reviewing = false;
    public array $cardIds = [];        // liste des ids dans l'ordre de révision
    public int $currentIndex = 0;
    public bool $flipped = false;      // true = traduction visible
    ...
    public function startReview(): void
    {
        ...
        if ($this->filterReference) {
            $query->where('reference', 'like', '%' . $this->filterReference . '%');
        }
        if ($this->filterPeriodFrom) {
            $query->where('period', '>=', $this->filterPeriodFrom);
        }
        if ($this->filterPeriodTo) {
            $query->where('period', '<=', $this->filterPeriodTo);
        }

        $ids = $query->pluck('id')->toArray();

        // Sélection aléatoire si demandée
        $n = (int) $this->randomCount;
        if ($n > 0 && $n < count($ids)) {
            shuffle($ids);
            $ids = array_slice($ids, 0, $n);
        } else {
            shuffle($ids); // ordre aléatoire par défaut
        }

        if (empty($ids)) {
            session()->flash('info', 'Aucune fiche ne correspond à votre sélection.');
            return;
        }
